...
const RangePages = (() => {
  ...
  function act(action,node){
    ...
    if(name==="key-submit"){const key=`sk-air-${Math.random().toString(16).slice(2,26)}`;state.keys.unshift({id:`key-${Date.now()}`,name:state.root.querySelector('#gw-name').value,prefix:key.slice(0,12),scope:"全部权限",quota:"0 / 50 万次",created:"2026-08-15",last:"从未使用",status:"active"});state.modal=modal("密钥已创建","仅此一次完整展示 · 请立即复制保存",`<div class="secret-key">${key}</div>`,button("我已保存","close-modal","primary",'id="gw-done"'));return rerender();}
